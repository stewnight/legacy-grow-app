        ...
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">
            {room?.building?.name ? (
              <>
                In{' '}
                <Link href={`/buildings/${room.building.id}`} className="hover:underline">
                  {room.building.name}
                </Link>
              </>
            ) : (
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {room?.properties?.temperature.min}° - {room?.properties?.temperature.max}°
              </div>
              ...
              <CardContent>
                <dl className="space-y-2">
                  ...
                  {room?.dimensions && (
                    <>
                      ...
                      {room?.dimensions.usableSqDimensions && (
                        <div>
                          <dt className="text-sm font-medium text-muted-foreground">Usable Area</dt>
                          <dd className="text-sm">
                            {room?.dimensions.usableSqDimensions} {room?.dimensions.unit}²
                          </dd>
                        </div>
                      )}
                    </>
                  ...
                  <div>
                    ...
                    <dd className="text-sm">
                      {formatDate(room?.createdAt ?? null)} by {room?.createdBy?.name}
                    </dd>
                  </div>
                </dl>
              </CardContent>
                  ...
                  <dl className="space-y-2">
                    {room?.properties.light && (
                      <div>
                        ...
                        <dd className="text-sm">
                          {room?.properties.light.type} - {room?.properties.light.intensity}%
                        </dd>
                      </div>
                    )}
                    {room?.properties.co2 && (
                      <div>
                        <dt className="text-sm font-medium text-muted-foreground">CO2 Range</dt>
                        <dd className="text-sm">
                          {room?.properties.co2.min} - {room?.properties.co2.max} ppm
                        </dd>
                      </div>
                    )}
                  </dl>
        ...
        <TabsContent value="children">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Rooms or tents contained within this room</CardDescription>
            </CardHeader>
            <CardContent>
              {room?.children && room?.children.length > 0 ? (
                <div className="space-y-4">
                  {room?.children.map((child) => (
                    ...
                    >
                      <div>
                        <Link href={`/rooms/${child.id}`} className="font-medium hover:underline">
                          {child.name}
                        </Link>
                        <p className="text-sm text-muted-foreground">{child.type}</p>
                      </div>
