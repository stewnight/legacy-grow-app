CREATE TYPE "public"."batch_status" AS ENUM('active', 'completed', 'pending', 'cancelled', 'archived');--> statement-breakpoint
CREATE TYPE "public"."building_type" AS ENUM('indoor', 'outdoor', 'greenhouse', 'hybrid');--> statement-breakpoint
CREATE TYPE "public"."common_status" AS ENUM('pending', 'in_progress', 'completed', 'cancelled', 'failed', 'on_hold', 'archived', 'maintenance');--> statement-breakpoint
CREATE TYPE "public"."destroy_reason" AS ENUM('died', 'pest', 'disease', 'male', 'hermaphrodite', 'quality', 'regulatory', 'other');--> statement-breakpoint
...
CREATE TYPE "public"."genetic_type" AS ENUM('sativa', 'indica', 'hybrid', 'ruderalis');--> statement-breakpoint
CREATE TYPE "public"."health_status" AS ENUM('healthy', 'sick', 'pest', 'nutrient', 'dead', 'quarantine');--> statement-breakpoint
CREATE TYPE "public"."job_category" AS ENUM('maintenance', 'transplanting', 'cloning', 'feeding', 'environmental', 'harvest', 'drying', 'trimming', 'packing', 'cleaning', 'inspection');--> statement-breakpoint
CREATE TYPE "public"."job_entity_type" AS ENUM('equipment', 'plant', 'batch', 'location', 'genetics', 'sensors', 'processing', 'harvest', 'none');--> statement-breakpoint
CREATE TYPE "public"."job_priority" AS ENUM('low', 'medium', 'high', 'urgent', 'critical');--> statement-breakpoint
CREATE TYPE "public"."job_status" AS ENUM('pending', 'in_progress', 'completed', 'cancelled', 'blocked', 'deferred');--> statement-breakpoint
CREATE TYPE "public"."location_type" AS ENUM('room', 'section', 'bench', 'shelf', 'tray', 'pot');--> statement-breakpoint
...
CREATE TYPE "public"."note_type" AS ENUM('text', 'voice', 'image', 'file');--> statement-breakpoint
CREATE TYPE "public"."plant_sex" AS ENUM('unknown', 'male', 'female', 'hermaphrodite');--> statement-breakpoint
CREATE TYPE "public"."plant_source" AS ENUM('seed', 'clone', 'mother', 'tissue_culture');--> statement-breakpoint
CREATE TYPE "public"."plant_stage" AS ENUM('germination', 'seedling', 'vegetative', 'flowering', 'harvested', 'mother', 'clone');--> statement-breakpoint
...
CREATE TYPE "public"."quality_grade" AS ENUM('A', 'B', 'C', 'D', 'F');--> statement-breakpoint
CREATE TYPE "public"."room_type" AS ENUM('vegetation', 'flowering', 'drying', 'storage', 'processing', 'mother', 'clone', 'quarantine');--> statement-breakpoint
CREATE TYPE "public"."sensor_type" AS ENUM('temperature', 'humidity', 'co2', 'light', 'ph', 'ec', 'moisture', 'pressure', 'airflow');--> statement-breakpoint
CREATE TYPE "public"."status" AS ENUM('active', 'inactive', 'archived', 'maintenance');--> statement-breakpoint
...
CREATE TYPE "public"."user_role" AS ENUM('user', 'admin', 'manager', 'viewer');--> statement-breakpoint
...
CREATE TABLE IF NOT EXISTS "legacy-grow-app_harvest" (
	...
	"location_id" uuid NOT NULL,
	"status" "common_status" DEFAULT 'pending' NOT NULL,
	"quality" "quality_grade",
	"wet_weight" numeric(10, 3) NOT NULL,
...
--> statement-breakpoint
CREATE TABLE IF NOT EXISTS "legacy-grow-app_job" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	...
	"started_at" timestamp with time zone,
	"completed_at" timestamp with time zone,
	"properties" json DEFAULT '{"recurring":null,"tasks":[],"instructions":[],"requirements":{"tools":[],"supplies":[],"ppe":[]}}'::json,
	"metadata" json DEFAULT '{"previousJobs":[],"nextJobs":[],"estimatedDuration":null,"actualDuration":null,"location":null}'::json,
	"created_by" uuid NOT NULL,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
...
CREATE TABLE IF NOT EXISTS "legacy-grow-app_processing" (
	...
	"method" "processing_method" NOT NULL,
	"status" "common_status" DEFAULT 'pending' NOT NULL,
	"quality" "quality_grade",
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "legacy-grow-app_harvest" ADD CONSTRAINT "legacy-grow-app_harvest_batch_id_legacy-grow-app_batch_id_fk" FOREIGN KEY ("batch_id") REFERENCES "public"."legacy-grow-app_batch"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "equipment_room_idx" ON "legacy-grow-app_equipment" USING btree ("room_id");--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "harvest_timing_idx" ON "legacy-grow-app_harvest" USING btree ("started_at","completed_at");--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "sensor_equipment_idx" ON "legacy-grow-app_sensor" USING btree ("equipment_id");--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "processing_quality_idx" ON "legacy-grow-app_processing" USING btree ("quality");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "processing_timing_idx" ON "legacy-grow-app_processing" USING btree ("started_at","completed_at");--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "note_created_at_idx" ON "legacy-grow-app_note" USING btree ("created_at");