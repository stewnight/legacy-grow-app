...
CREATE TABLE IF NOT EXISTS "legacy-grow-app_post" (
	"id" integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (sequence name "legacy-grow-app_post_id_seq" INCREMENT BY 1 MINVALUE 1 MAXVALUE 2147483647 START WITH 1 CACHE 1),
	"name" varchar(256),
...
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "legacy-grow-app_post" ADD CONSTRAINT "legacy-grow-app_post_created_by_legacy-grow-app_user_id_fk" FOREIGN KEY ("created_by") REFERENCES "public"."legacy-grow-app_user"("id") ON DELETE no action ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
...
CREATE INDEX IF NOT EXISTS "created_by_idx" ON "legacy-grow-app_post" USING btree ("created_by");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "name_idx" ON "legacy-grow-app_post" USING btree ("name");--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "session_user_id_idx" ON "legacy-grow-app_session" USING btree ("user_id");